...
#import "ETHomePagePKRecordView.h"
#import "ETHomePageViewModel.h"
#import "ETHomePagePKTableViewCell.h"
#import "ETNoDataTableViewCell.h"

@interface ETHomePagePKRecordView () <UITableViewDelegate, UITableViewDataSource>
...
@implementation ETHomePagePKRecordView
...
- (void)et_bindViewModel {
    @weakify(self)
//    [self.viewModel.refreshPKRecordCommand execute:nil];
    
    [self.viewModel.refreshPKRecordEndSubject subscribeNext:^(id x) {
        @strongify(self)
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return self.viewModel.pkRecordDataArray.count ? self.viewModel.pkRecordDataArray.count : 1;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.viewModel.pkRecordDataArray.count) {
        ...
        cell.pkRecordViewModel = self.viewModel.pkRecordDataArray[indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return self.viewModel.pkRecordDataArray.count ? 99 : self.mainTableView.jk_height;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.viewModel.pkRecordDataArray.count) {
        [self.viewModel.pkRecordCellClickSubject sendNext:[self.viewModel.pkRecordDataArray[indexPath.row] model]];
    }
}
