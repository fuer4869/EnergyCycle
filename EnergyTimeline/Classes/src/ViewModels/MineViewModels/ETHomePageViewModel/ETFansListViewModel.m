...
@interface ETFansListViewModel ()

@property (nonatomic, assign) NSInteger currentPage;

@end

@implementation ETFansListViewModel
    ...
    [self.refreshDataCommand.executionSignals.switchToLatest subscribeNext:^(id responseObject) {
        ...
        if ([responseObject[@"Status"] integerValue] == 200 && [responseObject[@"Data"] count]) {
            ...
            for (NSDictionary *dic in responseObject[@"Data"]) {
                ...
                ETFansListTableViewCellViewModel *viewModel = [[ETFansListTableViewCellViewModel alloc] init];
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                @strongify(self)
                self.currentPage = 1;
                User_Attention_Me_List_Request *fansListRequest = [[User_Attention_Me_List_Request alloc] initWithType:ETAllFans ToUserID:self.userID PageIndex:self.currentPage PageSize:10];
                [fansListRequest startWithCompletionBlockWithSuccess:^(__kindof ETBaseRequest * _Nonnull request) {
