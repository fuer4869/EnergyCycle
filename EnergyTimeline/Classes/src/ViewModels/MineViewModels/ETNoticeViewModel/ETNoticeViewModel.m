...
#import "Mine_Notice_List_Request.h"

@interface ETNoticeViewModel ()

@property (nonatomic, assign) NSInteger currentPage;

@end

@implementation ETNoticeViewModel
    ...
    [self.refreshDataCommand.executionSignals.switchToLatest subscribeNext:^(id responseObject) {
        ...
        if ([responseObject[@"Status"] integerValue] == 200 && [responseObject[@"Data"] count]) {
            ...
            for (NSDictionary *dic in responseObject[@"Data"]) {
                NoticeModel *model = [[NoticeModel alloc] initWithDictionary:dic error:nil];
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                @strongify(self)
                self.currentPage = 1;
                Mine_Notice_List_Request *noticeListRequest = [[Mine_Notice_List_Request alloc] initWithPageIndex:self.currentPage PageSize:10];
                [noticeListRequest startWithCompletionBlockWithSuccess:^(__kindof ETBaseRequest * _Nonnull request) {
