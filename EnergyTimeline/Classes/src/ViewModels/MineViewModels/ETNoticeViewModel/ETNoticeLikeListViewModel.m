...
#import "Mine_Notice_Like_List_Request.h"
#import "ETNoticeLikeListTableViewCellViewModel.h"

@interface ETNoticeLikeListViewModel ()

@property (nonatomic, assign) NSInteger currentPage;

@end

@implementation ETNoticeLikeListViewModel
    ...
    [self.refreshDataCommand.executionSignals.switchToLatest subscribeNext:^(id responseObject) {
        ...
        if ([responseObject[@"Status"] integerValue] == 200 && [responseObject[@"Data"] count]) {
            ...
            for (NSDictionary *dic in responseObject[@"Data"]) {
                ...
                ETNoticeLikeListTableViewCellViewModel *viewModel = [[ETNoticeLikeListTableViewCellViewModel alloc] init];
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                @strongify(self)
                self.currentPage = 1;
                Mine_Notice_Like_List_Request *likeListRequest = [[Mine_Notice_Like_List_Request alloc] initWithPageIndex:self.currentPage PageSize:10];
                [likeListRequest startWithCompletionBlockWithSuccess:^(__kindof ETBaseRequest * _Nonnull request) {
