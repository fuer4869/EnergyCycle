...
@interface ETMentionListViewModel ()

@property (nonatomic, assign) NSInteger currentPage;

@end

@implementation ETMentionListViewModel
    ...
    [self.refreshDataCommand.executionSignals.switchToLatest subscribeNext:^(id responseObject) {
        ...
        if ([responseObject[@"Status"] integerValue] == 200 && [responseObject[@"Data"] count]) {
            ...
            for (NSDictionary *dic in responseObject[@"Data"]) {
                ...
                ETMentionListTableViewCellViewModel *viewModel = [[ETMentionListTableViewCellViewModel alloc] init];
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
                @strongify(self)
                self.currentPage = 1;
                Mine_Notice_MenTion_List_Request *mentionListRequest = [[Mine_Notice_MenTion_List_Request alloc] initWithPageIndex:self.currentPage PageSize:10];
                [mentionListRequest startWithCompletionBlockWithSuccess:^(__kindof ETBaseRequest * _Nonnull request) {
