...
- (void)setPagesAtContentOffset:(CGPoint)offset{
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            if ([self.delegate respondsToSelector:@selector(locationChange:)]) {
                
                [self.delegate locationChange:self.visibleRange];
...
- (void)scrollToLastPage {
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            [_scrollView setContentOffset:CGPointMake(self.scrollView.contentOffset.x - _pageSize.width, 0) animated:YES];
            break;
        }
        case NewPagedFlowViewOrientationVertical:{
            
            [_scrollView setContentOffset:CGPointMake(0, self.scrollView.contentOffset.y - _pageSize.height) animated:YES];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    if ([self pointInside:point withEvent:event]) {
        ...
        for (UIView *subview in [_scrollView.subviews reverseObjectEnumerator]) {
            if (!_scrollView.isUserInteractionEnabled || _scrollView.isHidden || _scrollView.alpha <= 0.01) {
                return nil;
            }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if ([_delegate respondsToSelector:@selector(didEndDraggingWithPage:inFlowView:)]) {
        [_delegate didEndDraggingWithPage:pageIndex inFlowView:self];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([_delegate respondsToSelector:@selector(didEndDeceleratingWithPage:inFlowView:)]) {
        [_delegate didEndDeceleratingWithPage:pageIndex inFlowView:self];
    }
}
