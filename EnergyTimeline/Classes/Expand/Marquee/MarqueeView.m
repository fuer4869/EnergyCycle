...
-(instancetype)initWithFrame:(CGRect)frame withTitle:(NSArray *)titles{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        rect.origin.x = 10;
        rect.origin.y = 0;
        self.firstBtn = [self btnframe:rect  titleColor:_titleColor action:@selector(clickBtn:)];
...
//    rect.origin.y = 0;
    self.moreBtn = [self btnframe: rect titleColor:_titleColor action:@selector(clickBtn:)];
...
- (void)setTitleArr:(NSArray *)titleArr {
    NSMutableArray *MutableTitles = [NSMutableArray arrayWithArray:titleArr];
    NSString *str = @"";
    self.titles = MutableTitles;
