...
//// 左侧附加的按钮
//@property (strong, nonatomic) UIButton *leftExtraBtn;
...
- (void)addScrollLineOrCoverOrExtraBtn {
    ...
    if (self.segmentStyle.isShowLeftExtraButton) {
        [self addSubview:self.leftExtraBtn];
    }
}
...
- (void)leftExtraBtnOnClick:(UIButton *)leftExtraBtn {
    if (self.leftExtraBtnOnClick) {
        self.leftExtraBtnOnClick(leftExtraBtn);
    }
}
...
- (void)setupScrollViewAndExtraBtn {
    ...
    if (self.extraBtn && self.leftExtraBtn) {
        scrollW -= (extraBtnW * 2);
    } else if (self.extraBtn || self.leftExtraBtn) {
        scrollW -= extraBtnW;
    }
    ...
    //        scrollW = maxX;
    //    }
    if (self.leftExtraBtn) {
        self.scrollView.frame = CGRectMake(extraBtnW, 0.0, scrollW, self.zj_height);
    } else {
    ...
    if (self.extraBtn && self.leftExtraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW + extraBtnW, extraBtnY, extraBtnW, self.zj_height - 2*extraBtnY);
        self.leftExtraBtn.frame = CGRectMake(0.0, extraBtnY, extraBtnW, self.zj_height - 2*extraBtnY);
    } else if (self.extraBtn && !self.leftExtraBtn) {
...
- (void)setupScrollViewAndLeftExtraBtn {
    CGFloat extraBtnW = 44.0;
    CGFloat extraBtnY = 5.0;
    
    //    UILabel *lastLabel = _titleLabels.lastObject;
    //    CGFloat maxX = CGRectGetMaxX(lastLabel.frame) + 8;
    CGFloat scrollW = self.leftExtraBtn ? _currentWidth - extraBtnW : _currentWidth;
    //    if (maxX < _currentWidth) {
    //        scrollW = maxX;
    //    }
    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.zj_height);
...
//
    ...
    if (self.scrollView.contentSize.width != self.scrollView.bounds.size.width + contentSizeXOff) {// 需要滚动
        ...
        CGFloat leftExtraBtnW = self.leftExtraBtn ? self.extraBtn.zj_width : 0.0;
        CGFloat maxOffSetX = self.scrollView.contentSize.width - (_currentWidth - extraBtnW - leftExtraBtnW);
...
- (UIButton *)leftExtraBtn {
    
    if (!self.segmentStyle.showLeftExtraButton) {
        return nil;
    }
    if (!_leftExtraBtn) {
        UIButton *btn = [UIButton new];
        [btn addTarget:self action:@selector(leftExtraBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
        NSString *imageName = self.segmentStyle.leftExtraBtnBackgroundImageName ? self.segmentStyle.leftExtraBtnBackgroundImageName : @"";
        [btn setImage:[UIImage imageNamed:imageName] forState:UIControlStateNormal];
        btn.contentMode = UIViewContentModeScaleAspectFit;
        btn.backgroundColor = [UIColor clearColor];
        // 设置边缘的阴影效果
